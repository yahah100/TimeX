...
class TimeXModel(nn.Module):
    ...
    def __init__(
        ...
        self.prototypes = nn.Parameter(
            torch.randn(self.n_prototypes, self.d_z)
        )  # 2 defines mu and logvar
    ...
    def forward(self, src, times, captum_input=False):
        ...
        if self.ablation_parameters.archtype == "transformer":
            pred_regular, z_main, z_seq_main = self.encoder_main(
                src, times, captum_input=False, get_agg_embed=True
            )
        else:
            pred_regular, z_main = self.encoder_main(
                src, times, captum_input=False, get_embedding=True
            )

        if not self.ablation_parameters.g_pret_equals_g:
            z_seq = self.encoder_pret.embed(
                src, times, captum_input=False, aggregate=False
            )
        ...
        if self.ablation_parameters.archtype == "transformer":
            if self.ablation_parameters.equal_g_gt:
                pred_mask, z_mask, z_seq_mask = self.encoder_main(
                    exp_src, times, attn_mask=ste_mask_attn, get_agg_embed=True
                )
            else:
                pred_mask, z_mask, z_seq_mask = self.encoder_t(
                    exp_src, times, attn_mask=ste_mask_attn, get_agg_embed=True
                )
        else:
            if self.ablation_parameters.equal_g_gt:
                pred_mask, z_mask = self.encoder_main(
                    exp_src, times, get_embeddding=True
                )
            else:
        ...
        if self.ablation_parameters.ptype_assimilation:
            ...
            if not self.training:  # Only get indices if testing bc it's an expensive op
                ptype_inds = torch.cat(
                    [
                        match_m[i, :].nonzero(as_tuple=True)[0]
                        for i in range(match_m.shape[0])
                    ]
                )
            else:
                ptype_inds = []
    ...
    def init_prototypes(self, train, seed=None, stratify=True):
        ...
        if self.ablation_parameters.archtype == "transformer":
            ...
            _, z_p = self.encoder_main(
                Xp, times_p, captum_input=False, get_embedding=True
            )

        self.prototypes = torch.nn.Parameter(
            z_p.detach().clone()
        )  # Init prototypes to class (via parameter)
    ...
    def mask_probabilities_btf(self, mask):
        """Normalize the released mask generator's feature-dependent layout."""
        return mask.transpose(0, 1) if self.d_inp > 1 else mask
    ...
    def compute_loss(self, output_dict):
        return sum(
            self.loss_weight_dict[k] * v
            for k, v in self.loss_components(output_dict).items()
        )
