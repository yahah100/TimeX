...
def train_mv6_consistency(
    ...
    for epoch in range(num_epochs):
        ...
        for X, times, y, ids in train_loader:  # Need negative sampling here
            ...
            if not torch.isfinite(out).all():
                raise FloatingPointError(
                    f"Nonfinite TimeX prediction at epoch {epoch + 1}"
                )
            ...
            if opt_pred_mask:
                ...
                clf_pred_loss = js_divergence(
                    out_dict["pred_mask"].softmax(dim=-1),
                    out_dict["pred"].softmax(dim=-1),
                )
                clf_loss += clf_pred_loss
            ...
            if sim_criterion is not None:
                if label_matching and embedding_matching:
                    ...
                    if simclr_training:
                        neg_inds = basic_negative_sampling(
                            X, ids, dataX, num_negatives=num_negatives_simclr
                        )
                        n_inds_flat = neg_inds.flatten()
                        if max_batch_size_simclr_negs is None:
                            neg_embeddings = model.encoder_main.embed(
                                dataX[:, n_inds_flat, :],
                                dataT[:, n_inds_flat],
                                captum_input=False,
                            )
                        else:
                            _, neg_embeddings = batch_forwards_TransformerMVTS(
                                model.encoder_main,
                                dataX[:, n_inds_flat, :],
                                dataT[:, n_inds_flat],
                        ...
                        emb_sim_loss = sim_criterion[0](
                            conc_embeddings, org_embeddings, neg_embeddings
                        )
                        ...
                        if model.ablation_parameters.side_assimilation:
                            emb_ptype_sim_loss = sim_criterion[0](
                                org_embeddings, out_dict["ptypes"]
                            )
                    ...
                    raise ValueError(
                        "Either label_matching or embedding_matching should be true"
                    )
            else:
            ...
            if hasattr(model, "loss_components"):
                with torch.no_grad():
                    ...
                    terms = model.loss_components(out_dict)
                    diagnostics.append(
                        {
                            **{k: float(v) for k, v in terms.items()},
                            "mask_mean": float(p.mean()),
                            "mask_std": float(p.std(unbiased=False)),
                            "mask_min": float(p.min()),
                            "mask_max": float(p.max()),
                            "mask_below_01": float((p < 0.1).float().mean()),
                            "mask_above_09": float((p > 0.9).float().mean()),
                            "temporal_variation": float(
                                (p[:, 1:] - p[:, :-1]).abs().sum() / p.numel()
                            ),
                        }
                    )
            optimizer.step()
        ...
        if (len(label_sim_list) > 0) and (len(emb_sim_list) > 0):
            sim_s = [
                "{:.4f}".format(np.mean(emb_sim_list)),
                "{:.4f}".format(np.mean(label_sim_list)),
            ]
        else:
            sim_s = f"{sim:.4f}"
        ...
        if early_stopping:
            if selection_criterion is not None:
                ...
                cond = met > best_val_metric
        if not np.isfinite(float(met)) or not np.isfinite(float(f1)):
            raise FloatingPointError("Nonfinite TimeX validation result")
        if cond:
            best_val_metric = met
        ...
        history.append(
            {
                "epoch": epoch + 1,
                "classification": float(clf),
                "weighted_explanation": float(exp[0]),
                "weighted_consistency": float(sim),
                "label_consistency": float(np.mean(label_sim_list))
                if label_sim_list
                else None,
                "embedding_consistency": float(np.mean(emb_sim_list))
                if emb_sim_list
                else None,
                "validation_macro_f1": float(f1),
                "selection_metric": float(met),
                "selected_epoch": best_epoch + 1,
                **(
                    {
                        k: float(np.mean([d[k] for d in diagnostics]))
                        for k in diagnostics[0]
                    }
                    if diagnostics
                    else {}
                ),
        ...
        if save_path is not None:
            Path(str(save_path) + ".epochs.json").write_text(
    ...
    print(
        f"Best Epoch: {best_epoch + 1} \t Validation selection metric = {best_val_metric:.4f}"
    )
