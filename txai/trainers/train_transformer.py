...
def train(
    ...
    train_loss, val_auc = [], []
    max_val_auc, best_epoch = -float("inf"), 0
    for epoch in range(num_epochs):
        ...
        for X, times, y in train_loader:
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError(
                    f"Nonfinite predictor loss at epoch {epoch + 1}"
                )
            loss.backward()
            if any(
            ...
            if counterfactual_training:
                ...
                for i in range(masks.shape[0]):
                    cart = torch.cartesian_prod(torch.arange(T), torch.arange(d))[
                        : mask_nums[i]
                    ]
                ...
                out = model(
                    xmasked, times[x_inds, :], captum_input=True, show_sizes=show_sizes
                )
        ...
        with torch.no_grad():
                ...
                if isinstance(model, CNN) or isinstance(model, LSTM):
                    pred = torch.cat(
                        [
                            model(xb, tb)
                            for xb, tb in zip(
                                torch.split(X, validate_by_step, dim=1),
                    ...
                    pred, _ = batch_forwards_TransformerMVTS(
                        model, X, times, batch_size=validate_by_step
                    )
            else:
                # if detect_irreg:
            ...
            if not torch.isfinite(pred).all() or not torch.isfinite(val_loss):
                raise FloatingPointError("Nonfinite predictor validation")
            ...
            if regression:
                ...
                auc = f1_score(
                    y.cpu().numpy(),
                    pred.argmax(dim=1).detach().cpu().numpy(),
                    average="macro",
                )
        ...
        if (epoch + 1) % print_freq == 0:  # Print progress:
            ...
            print(
                "Epoch {}, Train Loss = {:.4f}, Val {} = {:.4f}".format(
                    epoch + 1, train_loss[-1], met, auc
                )
            )
    ...
    print(
        "Best validation {} at epoch {}: {:.4f}".format(
            "negative MAE" if regression else "macro-F1", best_epoch + 1, max_val_auc
        )
    )
