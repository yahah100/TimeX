...
def run_dynamask(
    ...
    mask = Mask(pert, device, task="classification", verbose=verbose)
...
def screen_dynamask(
    model, test_tuples, only_correct=True, device=None, dynamask_config=default_config
):
    """
    ...
    for X, time, y in test_tuples:
        ...
        mask = run_dynamask(
            model=model, X=X, time_input=time, **default_config, device=device
        )
