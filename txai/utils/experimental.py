...
def get_explainer(key, args, device=None):
    ...
    if key == "fit":
        ...
        explainer = partial(
            run_dynamask, device=device, verbose=getattr(args, "progress", True)
        )
        ...
        def explainer(model, x, time, y):
            ...
            out = TSR(
                GradExplainer, x, y, additional_forward_args=(time, None, True, False)
            )
...
def get_dataset(data, split, device=None):
    ...
    if data == "pam":
        train, val, test = process_PAM(
            split_no=split, device=device, base_path=dataset_path("PAM"), gethalf=True
        )

    elif (data == "epi") or (data == "epilepsy"):
        train, val, test = process_Epilepsy(
            split_no=split, device=device, base_path=dataset_path("Epilepsy")
        )

    elif data == "spike":
        D = process_Synth(
            split_no=split, device=device, base_path=dataset_path("Spike") / "simple"
        )
