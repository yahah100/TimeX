...
def eval_on_tuple(test_tuple, model, n_classes, mask=None):
    ...
    f1 = f1_score(
        y.cpu().numpy(), pred.argmax(dim=1).detach().cpu().numpy(), average="macro"
    )
...
def eval_mv2(test_tuple, model):
    model.eval()
    X, times, y = test_tuple
    pred, mask_in, ste_mask, smoother_stats, smooth_src = model(
        X, times, captum_input=False
    )
...
def eval_mv3(test_tuple, model):
    model.eval()
    X, times, y = test_tuple
    pred, pred_tilde, mask_in, ste_mask, smoother_stats, smooth_src = model(
        X, times, captum_input=False
    )
...
def eval_mv3_sim(test_tuple, model):
    model.eval()
    X, times, y = test_tuple
    pred, pred_tilde, mask_in, ste_mask, smoother_stats, smooth_src, zs = model(
        X, times, captum_input=False
    )
...
def eval_mvts_transformer(test_tuple, model, batch_size=None, auprc=False, auroc=False):
    ...
    if batch_size is not None:
        if isinstance(model, CNN) or isinstance(model, LSTM):
            ...
            pred, _ = batch_forwards_TransformerMVTS(
                model, X, times, batch_size=batch_size
            )
    else:
        pred = model(X, times)

    if not torch.isfinite(pred).all():
        raise FloatingPointError("Nonfinite classifier evaluation logits")
    ...
    if auroc:
        auroc_val = roc_auc_score(
            one_hot_y, pred_prob, average="macro", multi_class="ovr"
        )
