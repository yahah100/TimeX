...
def exp_criterion_evaluation(
    mask: torch.Tensor, beta: float, exp_criterion: torch.nn.Module
):
...
class SATGiniLoss(nn.Module):
    ...
    def forward(self, att, clf_logits, clf_labels):
        ...
        info_loss = torch.sum(
            torch.stack(
                [gini_loss(torch.log(att[i] + 1e-6)) for i in range(att.shape[0])]
            )
        )
...
class GiniLoss(nn.Module):
    ...
    def forward(self, att):
        # Must handle over batches:
        loss = -1.0 * torch.sum(
            torch.stack([gini_loss(att[i] + 1e-6) for i in range(att.shape[0])])
        )
        return loss
...
class GSATLoss(nn.Module):
    ...
    def forward(self, att):
        ...
        assert (att < 0).sum() == 0
        info_loss = (
            att * torch.log(att / self.r + 1e-6)
            + (1 - att) * torch.log((1 - att) / (1 - self.r + 1e-6) + 1e-6)
        ).mean()
        ##print(info_loss)
...
class GSATLoss_Extended(nn.Module):
    ...
    def forward(self, src, times, smoother_stats, att):
        ...
        if torch.any(att < 0):
            ...
            exit()
        info_loss = (
            att * torch.log(att / (self.r + 1e-6) + 1e-6)
            + (1 - att) * torch.log((1 - att) / (1 - self.r + 1e-6) + 1e-6)
        ).mean()
        if torch.any(torch.isnan(info_loss)):
            print("INFO LOSS NAN")
